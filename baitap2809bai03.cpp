...
class Account {
    ...
    double balance;
    public:
    Account(std::string number, std::string name, double initiaBalance)
    : accountNumber(number), ownerName(name), balance(initiaBalance) {}
    virtual void deposit(double amount) {
        if (amount > 0) {
            balance += amount;
            std::cout <<"Da nap" << amount << " vao tai khoan." << std::endl;
        } else {
            std::cout << "So tien khoong hop le." << std::endl;
        }
    }
...
int main() {
    Bank myBank;
    myBank.addAccount(new SavingsAccount("SA001", "Nguyen Van A", 1000000, 0.5));
